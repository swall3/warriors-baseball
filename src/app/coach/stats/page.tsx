...
export default function StatsPage() {
  ...
  const [score, setScore] = useState<{ us: number; opponents: number }>({
    us: 0,
    opponents: 0,
  });
  ...
  const loadStats = async () => {
    try {
      ...
      if (raw) {
        const gameState = JSON.parse(raw) as StoredGameState & {
          id?: string;
          gameId?: string;
          ourRuns?: number;
          oppRuns?: number;
        ...
        nextScore = {
          ...
          opponents:
            typeof gameState.oppRuns === "number" ? gameState.oppRuns : 0,
        };
        if (
          typeof gameState.opponentTeamName === "string" &&
          gameState.opponentTeamName.trim()
        ) {
      ...
      if (!response.ok)
        setLoadError(
          "Game reports unavailable. Showing only this device’s data.",
        );
      if (response.ok) {
        const data = await response.json();
        if (data.ok && Array.isArray(data.games)) {
          const apiGames = data.games as ApiGame[];
          pins = apiGames.flatMap((game) => game.pins ?? []);
          nextScore = apiGames.reduce(
            (a, g) => ({
          ...
          nextOpponentName = "All opponents";
        }
      }

      const usPins = pins.filter(
        (pin) => toTeamAtBat(pin.battingTeam) === "us",
      );

      // Calculate stats
      const totalPlays = usPins.length;
      const outs = usPins.filter(
        (p) =>
          p.result === "out" ||
          p.result === "fielders_choice" ||
          p.result === "strikeout",
      ).length;
      const hits = usPins.filter((p) =>
      ...
      usPins.filter(p=>isContact({...p,battingTeam:"us"})).forEach((pin) => {
  ...
  const handleExport = () => {
    try {
      ...
      lines.push("");
      lines.push(
        `**Score:** ${brandTeam.name} ${score.us} — ${opponentName} ${score.opponents}`,
      );
      if (stats) {
        lines.push(
          `**Total Plays:** ${stats.totalPlays} · **Hits:** ${stats.hits} · **Outs:** ${stats.outs} · **HR:** ${stats.homeRuns}`,
        );
        lines.push(
          `**Singles/Doubles/Triples:** ${stats.singles}/${stats.doubles}/${stats.triples}`,
        );
      ...
      for (const p of plays) {
        lines.push(
          `| ${p.inning} | ${p.batter} | ${p.result} | ${getZoneLabel(p.zone)} |`,
        );
      }
  ...
  if (loading) {
    return (
      <AnalyticsWorkspace
        ...
        description="Explore contact locations and results."
      >
  ...
  return (
    <AnalyticsWorkspace
      title="Where the ball goes."
      description="Explore contact locations, outcomes, and the combined scores across saved games."
    >
      {loadError && (
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <section className="overflow-hidden nf-card  lg:col-span-2">
          <header className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-d-sel">
                Spray Chart
              </p>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-d-line bg-d-surface px-3 py-2">
              ...
              <p className="text-2xl font-black text-d-sel">
                {stats?.totalPlays || 0}
              </p>
            </div>
            <div className="rounded-xl border border-d-line bg-d-surface px-3 py-2">
              <p className="text-[11px] text-d-ink-3">Hits</p>
              <p className="text-2xl font-black text-d-pos">
                {stats?.hits || 0}
              </p>
            </div>
            <div className="rounded-xl border border-d-line bg-d-surface px-3 py-2">
              <p className="text-[11px] text-d-ink-3">Outs</p>
              <p className="text-2xl font-black text-d-neg">
                {stats?.outs || 0}
              </p>
            </div>
            <div className="rounded-xl border border-d-line bg-d-surface px-3 py-2">
              <p className="text-[11px] text-d-ink-3">Home Runs</p>
              <p className="text-2xl font-black text-d-warn">
                {stats?.homeRuns || 0}
              </p>
            </div>
          </div>
          ...
          <div className="mb-4 grid grid-cols-4 gap-2 text-center">
            <div className="rounded-xl bg-d-surface px-2 py-1">
              ...
              <p className="text-lg font-bold text-d-pos">
                {stats?.singles || 0}
              </p>
            </div>
            <div className="rounded-xl bg-d-surface px-2 py-1">
            ...
            <div className="rounded-xl bg-d-surface px-2 py-1">
              ...
              <p className="text-lg font-bold text-d-sel">
                {getZoneLabel(stats?.favoriteZone || "N/A")}
              </p>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-d-line bg-d-bg">
            <table className="w-full text-left">
              ...
              <tbody className="text-xs">
                {plays.map((play, idx) => (
                  <tr
                    key={`${play.inning}-${play.batter}-${idx}`}
                    className="border-t border-d-line"
                  >
        ...
        <section className="overflow-hidden nf-card ">
          ...
          <div className="mt-4 flex items-center justify-center gap-6">
            ...
            <div className="text-center">
              ...
              <p className="text-3xl font-black text-d-neg">
                {score.opponents}
              </p>
            </div>
          </div>
