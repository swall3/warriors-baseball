"use client";
import { mergeReportGames } from "@/lib/coach/reports";
...
const DEFENSE_GROUP_NAMES: DefenseGroupName[] = [
  "A1",
  "A2",
  "B1",
  "B2",
...
function loadGames(stateKey: string, historyKey: string): StoredGame[] {
  ...
  try {
    ...
    if (raw) {
      ...
      if (
        typeof parsed?.opponentTeamName === "string" &&
        parsed.opponentTeamName.trim()
      ) {
  ...
  const normalizedHistory = historicalGames
    ...
    .map((g, idx) => ({
      ...
      label:
        g.label ||
        `Game ${idx + 1}${g.date ? ` (${toDateLabel(g.date)})` : ""}`,
      date: g.date,
      pins: g.pins || [],
      opponentTeamName:
        typeof g.opponentTeamName === "string" && g.opponentTeamName.trim()
          ? g.opponentTeamName.trim()
          : "Opponents",
    }));
...
function groupForInning(
  perInning: Record<number, DefenseGroupName> | null,
  inning: number,
): DefenseGroupName {
  if (perInning && perInning[inning]) return perInning[inning];
  return DEFENSE_GROUP_NAMES[
    (Math.max(inning, 1) - 1) % DEFENSE_GROUP_NAMES.length
  ];
}
...
function buildFairness(snapshot: DefenseSnapshot, innings = PLANNED_INNINGS) {
  const players = new Set<string>(
    snapshot.usLineup.map((name) => canonicalPlayerName(name)),
  );
  ...
  return playerList.length > 0
    ? playerList.map((player) => ({ player, counts: counts[player] }))
    : Object.keys(counts)
        .sort()
        .map((player) => ({ player, counts: counts[player] }));
}
...
function buildZoneStats(pins: StoredPin[], selectedTeam: TeamKey) {
  const isHit = (r: string) =>
    ["single", "double", "triple", "home_run", "hit"].includes(r);
  const isOutResult = (r: string) =>
    ["out", "strikeout", "fielders_choice"].includes(r);
  ...
  for (const p of pins) {
    if (teamOf(p) !== selectedTeam || !isContact({...p,battingTeam:teamOf(p)})) continue;
...
function buildPlayerReports(pins: StoredPin[], selectedTeam: TeamKey) {
  ...
  for (const pin of pins) {
    ...
    if (result === "double" || result === "triple" || result === "home_run")
      report.xbh += 1;
    if(isContact({...pin,battingTeam:teamOfPin(pin)})) report.zones[zone] = (report.zones[zone] || 0) + 1;
  }

  return Object.values(reports).sort(
    (a, b) => b.pa - a.pa || a.player.localeCompare(b.player),
  );
}
...
function buildContactSummary(pins: StoredPin[], selectedTeam: TeamKey) {
  ...
  const reached = teamPins.filter((pin) =>
    REACHED_RESULTS.has(pin.result),
  ).length;
  const atBats = teamPins.filter((pin) => pin.result !== "walk").length;
  const avg = atBats ? hits / atBats : 0;
  const obpDenominator =
    atBats + teamPins.filter((pin) => pin.result === "walk").length;
  const obp = obpDenominator
    ? (hits + teamPins.filter((pin) => pin.result === "walk").length) /
      obpDenominator
    : 0;
...
function buildMarkdownExport(
  ...
  brandTeam: { name: string },
): string {
  const pins = game.pins || [];
  const score = game.score || snapshot.score;
  const opponentTeamName =
    game.opponentTeamName || snapshot.opponentTeamName || "Opponents";
  ...
  lines.push(
    `**Score:** ${brandTeam.name} ${readScoreUs(score)} — ${opponentTeamName} ${score.opponents}`,
  );
  ...
  if (snapshot.groups && snapshot.perInning) {
    ...
    const header = [
      "Position",
      ...Array.from({ length: PLANNED_INNINGS }, (_, i) => `I${i + 1}`),
    ];
  ...
  if (pins.length > 0) {
    ...
    pins
      ...
      .forEach((p, idx) => {
        ...
        lines.push(
          `| ${idx + 1} | ${p.inning ?? "-"} | ${team} | ${canonicalPlayerName(p.batter) || "-"} | ${p.result} | ${ZONE_LABELS[zone] || zone} |`,
        );
      });
    lines.push("");
  }
...
function pinsToPlayEvents(
  ...
  return pins
    ...
    .filter(
      (p) =>
        !selectedPlayer ||
        canonicalPlayerName(p.batter || "") === selectedPlayer,
    )
    .map((p, i) => ({
      id: String(p.id ?? i),
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/coach/reports", {
      ...
      .then(async (r) => {
        ...
        if (!r.ok) throw new Error(data.error ?? "Game reports unavailable.");
        return data;
      })
      .then((data) => {
  ...
  const games = useMemo(
    () => mergeReportGames(localGames, serverGames),
    [localGames, serverGames],
  );
  const [selectedGameIds, setSelectedGameIds] = useState<string[]>(
    localGames.length ? [localGames[0].id] : [],
  );

  const selectedGames = useMemo(() => {
    if (scope === "all") return games.filter((g) => g.id !== "current");
  ...
  const scopedPins = useMemo(
    () => selectedGames.flatMap((g) => g.pins || []),
    [selectedGames],
  );
  ...
  const activePlayer =
    selectedPlayer !== "all" && playerOptions.includes(selectedPlayer)
      ? selectedPlayer
      : null;
  ...
  const selectedSummary = useMemo(
    () => buildContactSummary(scopedPins, selectedTeam),
    [scopedPins, selectedTeam],
  );
  const playerReports = useMemo(
    () => buildPlayerReports(scopedPins, selectedTeam),
    [scopedPins, selectedTeam],
  );
  const selectedZoneReport = useMemo(
    () => buildZoneReport(scopedPins, selectedTeam),
    [scopedPins, selectedTeam],
  );
  const opponentGapReport = useMemo(
    () => buildZoneReport(scopedPins, "them"),
    [scopedPins],
  );

  const defenseSnapshot = useMemo(
    () => loadDefense(storageKeys.state),
    [storageKeys.state],
  );
  const inningGrid = useMemo(
    () => buildInningGrid(defenseSnapshot),
    [defenseSnapshot],
  );
  const fairness = useMemo(
    () => buildFairness(defenseSnapshot),
    [defenseSnapshot],
  );
  ...
  const exportScopeGame = (): StoredGame => ({
    ...
    date: new Date().toISOString(),
    pins: scopedPins,
    score: selectedGames.reduce(
      (a, g) => ({
        us: a.us + (g.score?.us ?? 0),
        opponents: a.opponents + (g.score?.opponents ?? 0),
      }),
      { us: 0, opponents: 0 },
    ),
  ...
  const onExportMarkdown = () => {
    downloadBlob(
      `${brandTeam.slug}-game-summary.md`,
      "text/markdown",
      buildMarkdownExport(
        exportScopeGame(),
        scope === "current"
          ? defenseSnapshot
          : { ...defenseSnapshot, groups: null, perInning: null, usLineup: [] },
        brandTeam,
      ),
    );
  };
  ...
  const onExportCsv = () => {
    downloadBlob(
      `${brandTeam.slug}-plays.csv`,
      "text/csv",
      buildCsvExport(exportScopeGame()),
    );
  };

  return (
    <AnalyticsWorkspace
      title="See the story behind each play."
      description="Explore contact patterns, player reports, and defensive playing time."
    >
      {loadError && (
        <div className="nf-notice" role="alert">
          {loadError} Displayed data may be device-only or previously loaded.
          <button onClick={() => setReload((v) => v + 1)}>
            Retry historical games
          </button>
        </div>
      )}
      <div className="nf-report-actions">
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="nf-card">
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-d-ink-2">
              Team View
            </h2>
            ...
            {scope === "multiple" && (
              <div className="grid grid-cols-1 gap-2 rounded-lg border border-d-line bg-d-surface p-3 sm:grid-cols-2">
                {games.map((game) => (
                  <label
                    key={game.id}
                    className="flex items-center gap-2 text-sm text-d-ink"
                  >
                    <input
                      type="checkbox"
        ...
        <div className="nf-card">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <option value="all">
                All batters (
                {selectedTeam === "us" ? brandTeam.name : opponentTeamLabel})
              </option>
            ...
            <span className="text-xs text-d-ink-3">
              Showing {activePlayer ? activePlayer : "all players"} in current
              scope.
            </span>
          </div>
        </div>
        ...
        <div className="nf-card">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-d-ink-2">
            Field Heat Map
          </h2>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <section className="nf-card">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-d-ink-2">
              Team Contact Report
            </h2>
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
              ...
              <Metric
                label="Outs Logged"
                value={String(selectedSummary.outs)}
              />
            </div>
            <p className="mt-3 text-xs text-d-ink-3">
              Historical imports use estimated field coordinates when notes only
              say things like LC, up middle, or down 3B line.
            </p>
          </section>

          <section className="nf-card lg:col-span-2">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-d-ink-2">
              Where Balls Are Finding Grass
            </h2>
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <ZoneList
                title={
                  selectedTeam === "us"
                    ? `${brandTeam.name} Contact`
                    : `${opponentTeamLabel} Contact`
                }
                rows={selectedZoneReport}
              />
        ...
        <section className="nf-card">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-d-ink-2">
            Defensive Plan &amp; Playing Time
          </h2>
          <p className="mt-1 text-xs text-d-ink-3">
            Saved device-only game’s defensive plan. Shared game lineups and
            playing time are in Review.
          </p>
          <div className="mt-3 grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <table className="min-w-full text-left text-xs text-d-ink">
                ...
                <tbody>
                  {inningGrid.map((row) => (
                    <tr
                      key={`grid-${row.spot}`}
                      className="border-b border-d-line"
                    >
                      <td className="py-2 pr-3 font-semibold text-d-sel">
                        {row.spot}
                      </td>
                      {row.cells.map((cell) => (
                        <td
                          key={`grid-${row.spot}-${cell.inning}`}
                          className={`py-2 pr-3 text-center ${cell.player ? "" : "text-d-ink-3"}`}
                        >
                          {cell.player || "—"}
                        </td>
                      ))}
            ...
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-d-ink-3">
                Bench Fairness (innings benched)
              </h3>
              <ul className="mt-2 space-y-1 text-xs text-d-ink">
                {fairness.map((f) => {
                  const bench = f.counts.BENCH || 0;
                  const field = Object.entries(f.counts).reduce(
                    (sum, [spot, n]) => (spot === "BENCH" ? sum : sum + n),
                    0,
                  );
                  return (
                    <li
                      key={`fair-${f.player}`}
                      className="flex items-center justify-between gap-3 rounded border border-d-line bg-d-surface px-3 py-1.5"
                    >
                      <span className="font-medium text-d-ink">{f.player}</span>
                      <span className="text-d-ink-3 tabular-nums">
                        <span className="text-d-pos">{field}</span> in field /{" "}
                        <span
                          className={bench > 1 ? "text-d-warn" : "text-d-ink-3"}
                        >
                          {bench}
                        </span>{" "}
                        benched
                ...
                {fairness.length === 0 && (
                  <li className="text-d-ink-3">
                    No defense plan set for the current game yet.
                  </li>
                )}
              </ul>
            </div>
            ...
            <table className="min-w-full text-left text-xs text-d-ink">
              ...
              <tbody>
                {playerReports.map((row) => {
                  ...
                  const obp = obpDenominator
                    ? (row.hits + row.walks) / obpDenominator
                    : 0;
                  return (
                ...
                {playerReports.length === 0 && (
                  <tr>
                    <td colSpan={10} className="py-4 text-center text-d-ink-3">
                      No player events in this scope.
                    </td>
                  </tr>
                )}
              </tbody>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-d-line bg-d-surface p-3">
      <div className="text-[10px] font-semibold uppercase tracking-wide text-d-ink-3">
        {label}
      </div>
...
function ZoneList({
  ...
  title: string;
  rows: Array<{ zone: string; hits: number; outs: number; total: number }>;
}) {
  return (
    <div className="rounded-lg border border-d-line bg-d-surface p-3">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-d-ink-3">
        ...
        {rows.slice(0, 7).map((row) => (
          ...
          >
            ...
            <span className="text-d-ink-3">
              <span className="font-semibold text-d-pos">{row.hits}</span> hits
              / {row.total} contacts
            </span>
          </li>
        ))}
